...
const UserDashboard = () => {
  ...
  const sidebarItems = [
    ...
    { id: 'software', name: 'Software Dev', icon: <Code className="w-5 h-5" /> },
    { id: 'website', name: 'Web Dev', icon: <Globe className="w-5 h-5" /> },
    { id: 'video', name: 'Video Production', icon: <Video className="w-5 h-5" /> },
    { id: 'ads', name: 'Social Ads', icon: <Megaphone className="w-5 h-5" /> },
    { id: 'analytics', name: 'Analytics', icon: <BarChart3 className="w-5 h-5" /> },
    { id: 'projects', name: 'Projects', icon: <FileText className="w-5 h-5" /> },
  ...
  const ServiceCard = ({ service }) => (
    <div className="bg-gradient-to-br from-gray-900/40 via-slate-800/30 to-gray-900/40 backdrop-blur-xl border border-white/10 rounded-2xl p-6 hover:border-white/20 transition-all duration-300 group cursor-pointer"
      ...
      <div className="space-y-3">
        ...
        <div className="flex justify-between items-center pt-2">
          ...
          <div className="flex items-center">
            <span className="text-white font-medium">${service.revenue.toLocaleString()}</span>
  ...
  const ProjectCard = ({ project }) => {
    const getStatusColor = (status) => {
      switch (status) {
        ...
        case 'Active': return 'text-purple-400 bg-purple-400/10';
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-950 via-slate-900 to-gray-950">
      {/* Sidebar */}
      <div className={`fixed left-0 top-0 h-full bg-gradient-to-b from-gray-900/90 via-slate-900/80 to-gray-900/90 backdrop-blur-2xl border-r border-white/10 transition-all duration-300 z-40 ${
        ...
        <div className="p-6 border-b border-white/10">
          <div className="flex items-center space-x-3">
            ...
            {!sidebarCollapsed && (
              <div>
                <h1 className="text-xl font-bold text-white">IT Farm</h1>
        ...
        >
          <ChevronRight className={`w-5 h-5 text-white transition-transform duration-300 ${sidebarCollapsed ? '' : 'rotate-180'}`} />
        </button>
      </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-bold text-white mb-1">
                {activeService === 'overview' ? 'Dashboard Overview' : 
                 sidebarItems.find(item => item.id === activeService)?.name || 'Dashboard'}
              </h1>
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="w-10 h-10 bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 rounded-full flex items-center justify-center">
        ...
        <main className="p-6">
          ...
          {activeService !== 'overview' && (
            <div className="bg-gradient-to-br from-gray-900/40 via-slate-800/30 to-gray-900/40 backdrop-blur-xl border border-white/10 rounded-2xl p-8 text-center">
              <div className="mb-4">
                {sidebarItems.find(item => item.id === activeService)?.icon && (
                  <div className="w-16 h-16 bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 rounded-2xl flex items-center justify-center mx-auto mb-4">
                    {React.cloneElement(sidebarItems.find(item => item.id === activeService).icon, { className: 'w-8 h-8 text-white' })}
                  </div>
                )}
              </div>
              ...
              <p className="text-gray-400 mb-6">
                Detailed {sidebarItems.find(item => item.id === activeService)?.name.toLowerCase()} management and analytics coming soon.
              </p>
